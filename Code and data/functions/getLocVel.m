function [velocity] = getLocVel(data_all, ToPlotOrNot)

%Function for converting raw rotary encoder signal to velocity 
 ...
 for k = 1:length(data_all.treadmill)
    ...
    velocity_temp = C.*ratio; %Ratio of total wheel distance is speed at each time point
    
    %Apply moving average filter in a 500 ms window to smooth the trace a bit
    velocity_temp_mm = movmean(velocity_temp,5);

    if ToPlotOrNot == 1;
    
        figure(1)
        subplot(length(data_all.treadmill),1,k);
        plot(timevec_ds(1:end-1)./60, velocity_temp_mm, 'k');
        xlim([-inf inf])
        ylim([-inf max(velocity_temp_mm)*1.1])
        title("Recording # " + k + "");
    ...
    velocity.raw{1,k} = velocity_temp;
    velocity.smooth{1,k} = velocity_temp_mm;
    velocity.sampRate{1,k} = temp_sampRate/downsampFac;
    velocity.time_sec{1,k} = timevec_ds(1:end-1);
