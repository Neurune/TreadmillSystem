%% INFORMATION
% 
% The code analyzes N recording files (i.e., habituation days) of treadmill data from one mouse and computes a range of quantifications (e.g.,total locomotion distance, maximal locomotion speed etc.) 
...
%% Convert voltage signal to locomotion speed

[speed] = getLocSpeed(data_all, 1); %Arguments: data structure and plot or not

%% Compute metrics and quantifications from speed trace

[locomotionQuant] = getLocQuant(speed,1.5,2,1,1); %Arguments: speed structure, locomotion threshold (cm/s), min interval between locomotion bouts (s), minimum duration of locomotion bout (s) and plot or not 
